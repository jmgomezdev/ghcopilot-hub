...
Load this file for hot-path micro-optimizations: string/regex costs, async overhead, closure capture, and tight-loop deoptimization risks.

Do NOT load this file for type modeling decisions (`type` vs `interface`) or generic naming/style questions.

Quick navigation:

- String and regex costs → [String Building with Array Join](#string-building-with-array-join) and [Regular Expression in Loops](#regular-expression-in-loops)
- Async overhead → [Unnecessary Async Overhead](#unnecessary-async-overhead)
