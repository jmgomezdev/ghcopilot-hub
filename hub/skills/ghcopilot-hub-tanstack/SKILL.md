...
- Validate search params with `ghcopilot-hub-zod` in `validateSearch`.
...
- MANDATORY: also load `ghcopilot-hub-tanstack-query` for optimistic updates, infinite queries, retry tuning, or mutation
  lifecycle complexity.
- MANDATORY: also load `ghcopilot-hub-tanstack-router` for nested route composition, advanced search/state syncing, or route
  masking.
